...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BMS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BMS_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Bms API.
 */
export class Bms {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['BMS_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('BMS_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BmsError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Bms client with an apiKey option, like new Bms({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Bms.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BMS_LOG'), "process.env['BMS_LOG']", this) ??
      defaultLogLevel;
  ...
  static BmsError = Errors.BmsError;
